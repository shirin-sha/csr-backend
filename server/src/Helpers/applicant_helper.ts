...
import { client_url, activating_link_string, Mail, secretKey } from '../config/variables'
import { ActivationLink, resetPasswordMail } from '../config/mailTemplates'

import { resolve } from 'path'
import imageUpload from '../func/imageUpload';
import { addEventToAlgolio, addProjectToAlgolio, updateProjectToAlgolio } from '../func/algolio';
import mongoose from 'mongoose';

// registration
export const registration = (data: any, isIndividual: boolean, files: any) => {
   return new Promise(async (resolve, reject) => {
      // validate fetched data
      validator.applicant_register(data, isIndividual).then(async (response) => {
         ...
         // add to database 
         const userData = await new applicantSchema({
            type: data.type,
         ...
         userData.save(async (err, res) => {
            if (err) {
               ...
               // create activating token
               console.log('id :', res._id as any);
               ...
               signing(token_data).then(async (token) => {
                  ...
                  await userData.updateOne({ "activation_token": token })
                  ...
                  console.log('string :' + activating_link_string + user_id);
                  const role = 0
...
export const login = (userData) => {
   return new Promise((resolve, reject) => {
      ...
      validator.login_validator(userData).then(result => {
         ...
         try {
            applicantSchema.exists({ email: userData.email }, async (err, res) => {
               ...
               if (res === null || res === undefined) {
                  ...
                  console.log('id:', id);
                  applicantSchema.findById(id, { type: 1, first_name: 1, last_name: 1, email: 1, company_name: 1, mobile: 1, activated: 1, activation_token: 1, password: 1, blocked: 1 }, async (err, resp: any) => {
                     if (err) {
                        reject(err.message)
                     }
                     ...
                     if (resp.activated == null) {
                        console.log('activated ', resp.activated);

                        resolve({ success: false, message: 'Your account is not activated,Please check your email for activating link ' })
                     }
                     ...
                     validate_data(userData.password, hashcode).then(async (response) => {
                        ...
                        // make  token
                        console.log('user id:', (res as any)._id);
                        const token_data = {
                           id: (res as any)._id, role: 1  //1-applicant
                        }
                        signing(token_data).then(token => {
                           const user = resp
                           user.password = null
                           console.log('token created', token, 'user', user);
            ...
            resolve({ message: 'Server internal error' + error, success: false })
         }
   ...
   return new Promise((resolve, reject) => {
      ...
      jwt.verify(code, secretKey, (err, value) => {
         if (err) {
            ...
            applicantSchema.find({ activation_token: code, email: email }, async (err, res) => {
               try {
                  console.log('res:', res);
                  if (res === null) {
   ...
   return new Promise((resolve, reject) => {
      ...
      validator.project_validator(form_data).then(async (resp) => {
         ...
         await project_data.save(async (err, result) => {
            ...
            //   return response.status(200).json({ message: 'bphoto upload', success: true })

            // add owner refference
            await project_data.updateOne({ added_applicant: user_id }).then(res => {
            ...
            //updateProjectToAlgolio(project_data._id)
   ...
   return new Promise((resolve, reject) => {
      ...
      validator.event_validator(form_data).then((async (resp) => {
         ...
         await event_data.save(async (err, result) => {
            if (err) {
               console.log('error from eventdb');
               resolve({
                  message: 'database eventvalidation error :' + err,
                  success: false
               })
            ...
            console.log({ imgUpload });
            // add owner refference
            await event_data.updateOne({ added_applicant: user_id }).then(res => {
               console.log('response from update owner data', res);
            ...
            if (files) {
               ...
               if (isArray) {
                  await files.forEach(async (file: any) => {
                     fileUploader(file).then(async (response: any) => {
                        console.log('document data eventtuploaded id:', (response as any)._id);
...
export const reset_password_email = (email) => {
   return new Promise((resolve, reject) => {
      validator.resetPassword_email_validator(email).then(validation_res => {
         ...
         applicantSchema.findOne({ email: email }, async (err, result) => {
            if (err || result == null) {
               resolve({
                  success: false,
               ...
               signing({ email, role: 1 }).then(async (token) => {
                  // token store in db
                  await applicantSchema.updateOne({ email: email }, { token: token })
                  //send email
   ...
   return new Promise((resolve, reject) => {
      ...
      validator.update_password_validator(data).then(async (res) => {
         ...
         jwt.verify(data.token, secretKey, async (err, value) => {
            ...
            const hashedPassword = await bcrypt_data(data.password)
            console.log('hashed password:', hashedPassword);
            applicantSchema.updateOne({ token: data.token }, { password: hashedPassword }).then((res) => {
               if (res == null) {
                  resolve({
...
export const userProfile = async (req, res) => {
   try {
      ...
      const profile = await applicantSchema.findOne({ _id: user_id }).populate('documents')
...
export const userUpdate = async (req, res) => {
   try {
      const data = req.body
      console.log("idddddddddddd",req.user_id)
      // const file=req.file
      ...
      const file = (req as any).files?.img && (req as any).files.img
        ...
        const imgUpload = await applicantSchema.updateOne({_id:req.user_id},{$set:{img:response}})
...
export const sendActivateMail = (data) => {
   ...
   return new Promise(async (resolve, reject) => {
      ...
      const token_data = { email: data.email }
      const userData = await applicantSchema.findOne({ email: data.email })
      ...
      signing(token_data).then(async (token) => {
         console.log('activated token: ', token);
         // add activation token to database
         await applicantSchema.updateOne({ email: data.email }, { "activation_token": token, admin_approval: true })
...
export const getNotification = async (req, res) => {
   ...
   try {
      if (userId) {
         ...
         const notifications = await applicantSchema.aggregate([
            { $match: { _id: userId } },
...
export const changePassword = async (req, res) => {
   ...
   try {
      ...
      const userData: any = await applicantSchema.findOne({ _id: userId })
      ...
      if (userPasswordDb) {
         validate_data(currentPassword, userPasswordDb).then(async (response) => {
            ...
            if (!response === true) {
               ...
               const updatePassword = await applicantSchema.findByIdAndUpdate({ _id: userId }, { password: hashedPassword }, {
                  new: true,
               })
